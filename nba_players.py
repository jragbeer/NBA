...
doc = curdoc()
doc.title = 'NBA Player Analysis'
doc.clear() # clears the html page and gives the tab a name
...
def makediv(df, teams):
    colour1 = "#113672"
    colour2 = "#e82751"
    text = wrap_in_paragraphs('Player', colour1, 2) + wrap_in_paragraphs(f"{selectplayer.value}", colour2,4)+ \
           wrap_in_paragraphs('Teams played on: ', colour1, 2) + wrap_in_paragraphs(f"{', '.join(teams)}", colour2, 3) +\
            wrap_in_paragraphs('Games Played: ', colour1, 2) + wrap_in_paragraphs(f"""{int(df['G'].sum()):d} <font color="#e82751" size = "3"><b>  ({df.index.levels[1].values[0]:d}-{df.index.levels[1].values[-1]:d})</b></font>""", colour2,5)+ \
           wrap_in_paragraphs(f'Years Active: <font color={colour2} size = "5"><b> {df.index.levels[1].values[0]}</b></font>', colour1, 2) + \
           wrap_in_paragraphs(f'Career Average PPG: <font color={colour2} size = "5"><b> {df["ptsavg"].mean():.1f}</b></font>', colour1, 2) + \
           wrap_in_paragraphs(f'Career Average APG: <font color={colour2} size = "5"><b> {df["astavg"].mean():.1f}</b></font>', colour1, 2) + \
           wrap_in_paragraphs(f'Career Average RPG: <font color={colour2} size = "5"><b> {df["rebavg"].mean():.1f}</b></font>', colour1, 2)
...
data = cleandf(data)
list_of_images = os.listdir(path + 'static/images/nba_player_analysis/')
teams = list(data['Tm'].unique())
teams.remove(0)
teamsDict = {t: np.asarray(data[data['Tm'] == t]['Player'].unique()) for t in teams}
...
heatmap.arc([-4], [0], radius = [0.88], start_angle=[3*np.pi/2], end_angle=[np.pi/2], color='black')
heatmap.segment(x0=[-0.41, -0.41, 0.27, 0.27], y0=[0.42, -0.42, 0.42, -0.42,  ], x1=[-0.41, -0.41, 0.27, 0.27], y1=[0.48, -0.48, 0.48, -0.48,], color="black", line_width=2)
g_key = ''

map_options = GMapOptions(lat=lat, lng=lon, map_type="roadmap", zoom=3)
map_figure = gmap(g_key, map_options, title="Career Stops", width = 475, height = 350)
...
# checkbox_group
button_group = CheckboxGroup(labels=["Average PPG Lines", "Average RPG Lines", "Average APG Lines", "Average PER Lines"], active=[], )
