...
def position_matters(data):
    ...
    # af['year_number'] = [x+1 for x in range(len())]
    print(af)
...
def improving_ft(data):
    ...
    cool = pd.DataFrame(list_of_players, index = af.unstack().index, columns = ['Season_{}'.format(x+1) for x in range(10)])
    ...
    for x in cool.itertuples():
        ...
        for i in range(10):
            try:
                nice[str(i+1)].append(af.at[(x.Index, yy[str(i+1)]), 'percent'])
            except:
    ...
    insane = pd.DataFrame.from_dict(nice)
    insane.index = cool.index
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
